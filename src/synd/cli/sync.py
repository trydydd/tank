"""synd sync command — reproduce the local index from synd.lock."""
...
def sync(policy: Path | None, frozen: bool) -> None:
    """Import all packs listed in synd.lock that are not yet in the local index.

    Reads synd.lock from the current directory, verifies each pack's digest
    ...
    try:
        ...
        console.print(f"[yellow]warning: could not update synd.lock: {exc}[/yellow]")
